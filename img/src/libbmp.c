               ...
               const char    *filename)
{
	...
	for (size_t y = 0; y < h; y++)
	{
		...
		fwrite (img->img_pixels[(size_t)abs((int)offset - (int)y)], sizeof (bmp_pixel), img->img_header.biWidth, img_file);
              ...
              const char *filename)
{
	...
	for (size_t y = 0; y < h; y++)
	{
		...
		if (fread (img->img_pixels[abs ((int)offset - (int)y)], sizeof (bmp_pixel), items, img_file) != items)
		{
			fclose (img_file);
			return BMP_ERROR;
		}
